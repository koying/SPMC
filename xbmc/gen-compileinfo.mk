...
$(FILEPATH)/CompileInfo.cpp: $(VERSION.TXT) $(FILEPATH)/CompileInfo.cpp.in $(FILEPATH)/.GitRevision
	...
	APP_PACKAGE_ROOT=$$(awk '/APP_PACKAGE_ROOT/ {print $$2}' $(VERSION.TXT)) ;\
	...
	sed -e "s/\@APP_NAME\@/$$APP_NAME/" -e "s/\@APP_PACKAGE_ROOT\@/$$APP_PACKAGE_ROOT/" -e "s/\@APP_VERSION_MAJOR\@/$$MAJOR/" -e "s/\@APP_VERSION_MINOR\@/$$MINOR/" -e "s/\@APP_VERSION_TAG\@/$$TAG/" -e "s/\@APP_SCMID\@/$$GITREV/" $@.in > $@

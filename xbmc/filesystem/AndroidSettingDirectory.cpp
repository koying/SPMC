...
bool CAndroidSettingDirectory::GetDirectory(const CStdString& strPath, CFileItemList &items)
{
  CURL url(strPath);
  CStdString dirname = url.GetFileName();
  URIUtils::RemoveSlashAtEnd(dirname);
  CLog::Log(LOGDEBUG, "CAndroidSettingDirectory::GetDirectory: %s",dirname.c_str());
  if (dirname == "settings")
  {
  ...
  CLog::Log(LOGERROR, "CAndroidSettingDirectory::GetDirectory Failed to open %s",strPath.c_str());
  return false;
}

bool CAndroidSettingDirectory::IsAllowed(const CStdString& strFile) const
{

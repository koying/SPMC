...
#define AV_SYNC_THRESH  AML_PTS_FREQ*30
...
bool CAMLCodec::OpenDecoder(CDVDStreamInfo &hints)
{
  ...
  am_private->video_pid        = hints.uniqueId;
  ...
  if (strScaler.find("enabled") == std::string::npos)     // Scaler not enabled, use screen size
  {
    ...
    if (SysfsUtils::GetString("/sys/class/video/device_resolution", displaySize) == 0)
    {
      std::vector<std::string> aSize = StringUtils::Split(displaySize, "x");
      if (aSize.size() == 2)
      {
        int width = StringUtils::IsInteger(aSize[0]) ? atoi(aSize[0].c_str()) : 0;
        int height = StringUtils::IsInteger(aSize[1]) ? atoi(aSize[1].c_str()) : 0;
        if (width && height)
          m_display_rect = CRect(0, 0, width, height);
      }
    }
  }
...
int CAMLCodec::Decode(uint8_t *pData, size_t iSize, double dts, double pts)
{
  ...
  if (pData)
  {
    ...
    {
      am_private->am_pkt.avpts = 0.5 + (pts * AML_PTS_FREQ) / DVD_TIME_BASE;\
    ...
    {
      am_private->am_pkt.avdts = 0.5 + (dts * AML_PTS_FREQ) / DVD_TIME_BASE;
...
//    rtn, m_cur_pictcnt, (float)m_cur_pts/AML_PTS_FREQ, (float)am_private->am_pkt.lastpts/AML_PTS_FREQ, GetTimeSize(), GetDataSize());
...
bool CAMLCodec::GetPicture(DVDVideoPicture *pDvdVideoPicture)
{
  ...
  double curclock = (pDvdVideoPicture->clock->GetClock() + (AML_NUM_BUFFERS * pDvdVideoPicture->iDuration)) / DVD_TIME_BASE;  // Adjust for buffers
...
//    app_pts, (double)pts_video/AML_PTS_FREQ);

  double error = app_pts - (double)pts_video/AML_PTS_FREQ;
...
//    CLog::Log(LOGDEBUG, "CAMLCodec::Process pts diff = %f", error);
    ...
    else
    {
      ...
      SetVideoPtsSeconds((double)pts_video/AML_PTS_FREQ + error/4);
    }
  }
  ...
  else
  {
    ...
    if (m_cur_pts == 0)
      pDvdVideoPicture->pts = (double)m_1st_pts / AML_PTS_FREQ * DVD_TIME_BASE;
    else
...
double CAMLCodec::GetTimeSize()
{
  ...
  if (m_cur_pts == 0)
    m_timesize = (double)(am_private->am_pkt.lastpts - m_1st_pts) / AML_PTS_FREQ;
  else
    m_timesize = (double)(am_private->am_pkt.lastpts - m_cur_pts) / AML_PTS_FREQ;
...
//          pts_video, (double)pts_video/AML_PTS_FREQ, 1.0/((double)(pts_video - m_cur_pts)/AML_PTS_FREQ));

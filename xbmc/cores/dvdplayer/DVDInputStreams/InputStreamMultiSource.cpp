...
CInputStreamMultiSource::CInputStreamMultiSource(IDVDPlayer* pPlayer, const std::vector<std::string>& filenames) : InputStreamMultiStreams(DVDSTREAM_TYPE_MULTIFILES),
  m_pPlayer(pPlayer),
...
bool CInputStreamMultiSource::Open()
{
  ...
  for (unsigned int i = 0; i < m_filenames.size(); i++)
  {
    ...
    InputStreamPtr inputstream(CDVDFactoryInputStream::CreateInputStream(m_pPlayer, m_filenames[i], filemimetype, false));
    if (!inputstream)
    {
    ...
    if (!inputstream->Open(m_filenames[i].c_str(), filemimetype, false))
    {

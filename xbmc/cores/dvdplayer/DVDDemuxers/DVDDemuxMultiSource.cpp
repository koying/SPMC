...
CDemuxStream* CDemuxMultiSource::GetStream(int iStreamId)
{
  int i = 0;
  DemuxPtr mux;
  for (auto m : m_pDemuxers)
  {
    if (i + mux->GetNrOfStreams() >= iStreamId)
    {
      mux = m;
      break;
    }
    i += mux->GetNrOfStreams();
  }

  if (!mux)
    return NULL;

  return mux->GetStream(iStreamId - i);
}
...
void CDemuxMultiSource::GetStreamCodecName(int iStreamId, std::string &strName)
{
  strName = "";
  ...
  mux->GetStreamCodecName(iStreamId - i, strName);
}

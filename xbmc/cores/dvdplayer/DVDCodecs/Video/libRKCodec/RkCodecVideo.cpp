...
#define RK_FBIOSET_VSYNC_ENABLE     0x4629
  ...
  if (ret || mVpuCtx == NULL) {
    CLog::Log(LOGERROR, "%s::%s - %s\n", CLASSNAME, __func__,"Cannot instantiate vpu context");
    return false;
  }
...
//  case AV_CODEC_ID_H264MVC:
  ...
  if(mVpuCtx->init(mVpuCtx, mExtraData, mExtraDataSize)){
    CLog::Log(LOGERROR, "%s::%s - %s\n", CLASSNAME, __func__,"Cannot initialize vpu");
    return false;
  }
  ...
  if (mExtraData && mCodecId == AV_CODEC_ID_VC1)
...
int  CRkCodecVideo::Decode(uint8_t *pData, int iSize, double dts, double pts)
{
  ...
  if (pData)
  {
    if (mCurPacket.pData == NULL)
    {
      if (mCodecId == OMX_ON2_VIDEO_CodingMPEG2)
      {
        ...
        mCurPacket.pData = malloc(iSize + sizeof(VPU_BITSTREAM));
  ...
  if(vret != 0)
  {
    SAFE_DELETE(mCurPacket.pData);
    return VC_ERROR;
  }

  memset(&mPicture, 0, sizeof(DecoderOut_t));
  uint8_t *frame = malloc(sizeof(VPU_FRAME));
  mPicture.data = frame;
  mPicture.size = 0;

  vret = mVpuCtx->decode_getframe(mVpuCtx, &mPicture);
  if (vret != 0)
  {
    free(mPicture.data);
    return VC_ERROR;
  ...
  if (mPicture.size)
  {
    ...
    VPU_FRAME *frame = (VPU_FRAME *)mPicture.data;
...
void CRkCodecVideo::RenderUpdateCallBack(const void *ctx, const CRect &SrcRect, const CRect &DestRect, const void* render_ctx)
{

...
{ 
  private static final String TAG = "SPMCproperties";
  ...
  {
    String propfn = Environment.getExternalStorageDirectory().getAbsolutePath() + "/spmc_env.properties";
    File fProp = new File(propfn);
    if (!fProp.exists())
    {
      propfn = Environment.getExternalStorageDirectory().getAbsolutePath() + "/xbmc_env.properties";
      fProp = new File(propfn);
    }
    if (fProp.exists())
    {
